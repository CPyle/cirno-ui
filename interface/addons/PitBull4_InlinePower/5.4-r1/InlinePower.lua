...
if select(6, _G.GetAddOnInfo("PitBull4_" .. (debugstack():match("[o%.][d%.][u%.]les\\(.-)\\") or ""))) ~= "MISSING" then return end
local PitBull4 = _G.PitBull4;
if not PitBull4 then error("PitBull4_InlinePower requires PitBull4") end
...
local _classes = {
	...
	["SHAMAN2"] = function(t)
		local has_maelstrom = _G.UnitLevel("player") >= 50;
		
		t._max = 5;
		t.spell = 77661; -- Searing Flames
		t.color = {r = 1, g = 0.2, b = 0};
		t.reqlevel = 34; -- Searing Flames will become active here
		
		if( has_maelstrom ) then
			t._max = 10; -- 5x each Maelstrom Weapon and Searing Flames
			t.spell2 = 53817; -- Maelstrom Weapon
			t.color_override[1] = {r = 0.3, g = 0.6, b = 1};
		end
